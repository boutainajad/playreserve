...
@section('title', 'Book ' . $terrain->name)
    ...
    @if(session('error'))
        <div class="mb-8 p-6 bg-red-50 border-l-8 border-red-500 text-red-800 rounded-2xl font-black flex items-center gap-4 animate-fadeInUp shadow-sm">
            ...
            <div>
                ...
                <div class="text-lg leading-tight">{{ session('error') }}</div>
            </div>
        </div>
    @endif
    ...
    <div class="mb-10 text-center md:text-left flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
            <nav class="flex items-center gap-2 text-sm font-bold text-gray-400 mb-4 tracking-wide uppercase">
                <a href="{{ route('home') }}" class="hover:text-playtomic-blue transition-colors">Home</a>
                <i class="bi bi-chevron-right text-[10px]"></i>
                <a href="{{ route('clubs.show', $terrain->club->id) }}" class="hover:text-playtomic-blue transition-colors">{{ $terrain->club->name }}</a>
                <i class="bi bi-chevron-right text-[10px]"></i>
                <span class="text-playtomic-blue">{{ $terrain->name }}</span>
            </nav>
        ...
        <div class="w-20 h-20 bg-playtomic-blue/10 rounded-3xl flex items-center justify-center text-playtomic-blue text-3xl">
             @if($terrain->sport_type == 'football') <i class="bi bi-dribbble"></i>
             @elseif($terrain->sport_type == 'basketball') <i class="bi bi-basket"></i>
             @elseif($terrain->sport_type == 'piscine') <i class="bi bi-water"></i>
                ...
                <div class="space-y-6">
                    <div class="flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <div class="font-bold text-[#0B1526]">{{ $terrain->club->name }}</div>
                        </div>
                    </div>
                    ...
                    <div class="flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <div class="font-bold text-[#0B1526]">{{ \Carbon\Carbon::parse($date)->translatedFormat('l, d M Y') }}</div>
                        </div>
                    </div>
