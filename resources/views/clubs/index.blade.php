...
@section('title', 'Sports Clubs')

@section('content')

<div class="mb-12 bg-white border border-gray-200 rounded-[32px] p-8 md:p-10 shadow-sm border-b-8 border-b-playtomic-blue">
    <div class="max-w-2xl">
        <h1 class="text-4xl font-black text-[#0B1526] tracking-tight mb-4">Find your club</h1>
        <p class="text-gray-500 text-[17px] font-medium mb-8">Discover the best multi-sport clubs in Morocco and book your court.</p>
    </div>

    <form action="{{ route('clubs.index') }}" method="GET" class="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div class="relative group">
            ...
            <input type="text" name="city" value="{{ request('city') }}" placeholder="Enter a city (e.g. Casablanca)..." 
        ...
        <div class="relative group">
            ...
            <select name="sport_type" class="w-full pl-11 pr-4 py-4 bg-[#f4f5f7] border-transparent rounded-2xl focus:bg-white focus:outline-none focus:ring-2 focus:ring-playtomic-blue font-bold appearance-none transition-all cursor-pointer">
                ...
                <option value="handball" {{ request('sport_type') == 'handball' ? 'selected' : '' }}>🤾 Handball</option>
                <option value="piscine" {{ request('sport_type') == 'piscine' ? 'selected' : '' }}>🏊 Swimming</option>
            </select>
        ...
        <div class="flex gap-2">
            ...
            @if(request()->anyFilled(['city', 'sport_type']))
                <a href="{{ route('clubs.index') }}" class="w-14 bg-gray-100 text-gray-500 hover:bg-gray-200 hover:text-gray-700 font-bold rounded-2xl flex items-center justify-center transition-all">
...
<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        ...
        <div class="bg-white border border-gray-200 rounded-[32px] overflow-hidden flex flex-col group relative shadow-sm hover:shadow-xl hover:translate-y-[-4px] transition-all duration-500">
                ...
                <div class="absolute bottom-4 left-6 flex flex-wrap gap-2 z-10">
                    ...
                    @foreach($sports as $sport)
                        <span class="bg-white/90 backdrop-blur-md px-3 py-1.5 rounded-full text-[10px] font-black uppercase tracking-wider text-playtomic-blue shadow-sm border border-playtomic-blue/20 flex items-center gap-1.5">
                            @if($sport == 'football') <i class="bi bi-dribbble"></i>
                            @elseif($sport == 'basketball') <i class="bi bi-basket"></i>
                            @elseif($sport == 'piscine') <i class="bi bi-water"></i>
                            @else <i class="bi bi-lightning-fill"></i> @endif
                            {{ $sport }}
                        </span>
                    @endforeach
                </div>
            ...
            <div class="p-8 flex flex-col flex-1">
                <div class="flex justify-between items-start mb-4">
                    <div>
                        <h3 class="text-2xl font-black text-[#0B1526] leading-tight mb-1">{{ $club->name }}</h3>
                        <p class="text-gray-400 font-bold text-[13px] uppercase tracking-wide">
                            {{ $club->terrains->count() }} Available Courts
                        </p>
                    </div>
                </div>
                ...
                <div class="mt-auto pt-6 border-t border-gray-100 flex items-center justify-between gap-4">
                    <div class="flex flex-col">
                        ...
                        <span class="text-lg font-black text-[#0B1526]">
                             {{ number_format($club->terrains->min('price_per_hour') ?? 0, 0) }} <span class="text-xs">DH</span>
                        </span>
                    </div>
                    <a href="{{ route('clubs.show', $club->id) }}" class="px-6 py-3.5 bg-playtomic-blue text-white font-black rounded-2xl text-[14px] hover:bg-blue-700 transition-all shadow-md shadow-playtomic-blue/10 flex items-center gap-2 group-hover:gap-3">
    ...
    @empty
        <div class="col-span-full flex flex-col items-center justify-center py-24 bg-white border border-gray-100 rounded-[40px] text-center shadow-sm">
            ...
            <h4 class="text-2xl font-black text-[#0B1526] mb-2">No results found</h4>
            ...
            <a href="{{ route('clubs.index') }}" class="mt-8 text-playtomic-blue font-black underline underline-offset-8">Clear all filters</a>
        </div>
    @endforelse
</div>
@endsection