...
<head>
    ...
    <title>PlayReserve — <?php echo $__env->yieldContent('title'); ?></title>
...
<body class="font-sans text-playtomic-text bg-playtomic-bg antialiased flex flex-col min-h-screen">
    ...
    <nav class="hidden md:flex sticky top-0 z-50 bg-white border-b border-gray-100 items-center w-full px-6 lg:px-12 py-4 h-[72px]">
        <div class="max-w-[1440px] mx-auto w-full flex items-center justify-between">
            <a class="flex items-center gap-2 text-playtomic-blue font-black text-xl tracking-wider uppercase" href="<?php echo e(auth()->check() ? route('dashboard') : route('home')); ?>">
            ...
            <ul class="flex items-center gap-6 ml-12 font-semibold text-[15px] text-gray-500">
                <?php if(auth()->user()->role === 'admin'): ?>
                    <li><a class="hover:text-playtomic-blue transition-colors px-2 py-1 <?php echo e(request()->routeIs('admin.dashboard') ? 'text-playtomic-blue' : ''); ?>" href="<?php echo e(route('admin.dashboard')); ?>">Dashboard</a></li>
                <?php elseif(auth()->user()->role === 'owner'): ?>
                    <li><a class="hover:text-playtomic-blue transition-colors px-2 py-1 <?php echo e(request()->routeIs('owner.dashboard') ? 'text-playtomic-blue' : ''); ?>" href="<?php echo e(route('owner.dashboard')); ?>">Dashboard</a></li>
                    <li><a class="hover:text-playtomic-blue transition-colors px-2 py-1" href="#">Courts</a></li>
                <?php else: ?>
                    <li><a class="hover:text-playtomic-blue transition-colors px-2 py-1 <?php echo e(request()->routeIs('dashboard') ? 'text-playtomic-blue' : ''); ?>" href="<?php echo e(route('dashboard')); ?>">Explore</a></li>
                    <li><a class="hover:text-playtomic-blue transition-colors px-2 py-1 <?php echo e(request()->routeIs('reservations.history') ? 'text-playtomic-blue' : ''); ?>" href="<?php echo e(route('reservations.history')); ?>">Matches</a></li>
                <?php endif; ?>
            </ul>
            <?php endif; ?>
            ...
            <div class="ml-auto flex items-center gap-4">
                ...
                <?php else: ?>
                    <a class="text-sm font-bold text-gray-500 hover:text-playtomic-blue transition-colors" href="<?php echo e(route('login')); ?>">Log in</a>
                    <a class="ml-2 flex items-center justify-center gap-2 bg-playtomic-blue text-white px-5 py-2.5 rounded-full font-bold hover:bg-blue-700 transition-colors shadow-md shadow-playtomic-blue/20" href="<?php echo e(route('register')); ?>">
                        Sign up
                    </a>
                <?php endif; ?>
            </div>
    ...
    <div class="md:hidden flex items-center justify-between px-4 h-16 bg-white sticky top-0 z-40 border-b border-gray-100 shadow-sm">
        <a class="flex items-center gap-2 text-playtomic-blue font-black text-lg uppercase" href="<?php echo e(route('home')); ?>">
    ...
    <main class="flex-1 w-full max-w-[1240px] mx-auto md:p-8 p-4 md:pb-20 pb-28">
        <?php if(session('success')): ?>
            <div class="flex items-center gap-3 p-4 mb-8 rounded-2xl bg-playtomic-lime/20 border border-playtomic-lime text-[#6A8100]">
                ...
                <p class="text-[15px] font-bold"><?php echo e(session('success')); ?></p>
            </div>
        <?php endif; ?>
        <?php if(session('error')): ?>
            <div class="flex items-center gap-3 p-4 mb-8 rounded-2xl bg-red-50 border border-red-100 text-red-600">
                ...
                <p class="text-[15px] font-bold"><?php echo e(session('error')); ?></p>
            </div>
        <?php endif; ?>
    ...
    <nav class="md:hidden fixed bottom-0 left-0 w-full h-16 bg-white border-t border-gray-200 flex justify-around items-center z-50 pb-safe shadow-[0_-4px_10px_rgba(0,0,0,0.03)]">
        <?php if(auth()->guard()->check()): ?>
            <a href="<?php echo e(route('home')); ?>" class="flex flex-col items-center justify-center w-full h-full text-gray-400 hover:text-playtomic-blue <?php echo e(request()->routeIs('home') ? 'text-playtomic-blue' : ''); ?> transition-colors relative">
                <?php if(request()->routeIs('home')): ?> <div class="absolute top-0 w-8 h-1 bg-playtomic-blue rounded-b-md"></div> <?php endif; ?>
                <i class="bi bi-house-door-fill text-xl mb-0.5"></i>
                <span class="text-[10px] font-bold">Home</span>
            </a>
            <a href="<?php echo e(route('dashboard')); ?>" class="flex flex-col items-center justify-center w-full h-full text-gray-400 hover:text-playtomic-blue <?php echo e(request()->routeIs('dashboard') ? 'text-playtomic-blue' : ''); ?> transition-colors relative">
                <?php if(request()->routeIs('dashboard')): ?> <div class="absolute top-0 w-8 h-1 bg-playtomic-blue rounded-b-md"></div> <?php endif; ?>
                <i class="bi bi-search text-xl mb-0.5"></i>
                <span class="text-[10px] font-bold">Explore</span>
            </a>
            <a href="<?php echo e(route('reservations.history')); ?>" class="flex flex-col items-center justify-center w-full h-full text-gray-400 hover:text-playtomic-blue <?php echo e(request()->routeIs('reservations.history') ? 'text-playtomic-blue' : ''); ?> transition-colors relative">
                <?php if(request()->routeIs('reservations.history')): ?> <div class="absolute top-0 w-8 h-1 bg-playtomic-blue rounded-b-md"></div> <?php endif; ?>
            ...
            <div class="flex flex-col items-center justify-center w-full h-full text-gray-400 cursor-pointer hover:text-playtomic-blue transition-colors" onclick="document.getElementById('mobile-logout').submit();">
                ...
                <form id="mobile-logout" method="POST" action="<?php echo e(route('logout')); ?>" class="hidden"><?php echo csrf_field(); ?></form>
            </div>
        <?php else: ?>
            <a href="<?php echo e(route('login')); ?>" class="flex flex-col items-center justify-center w-full h-full text-gray-500 font-bold text-[13px] hover:text-playtomic-blue">
                Log in
            </a>
            <a href="<?php echo e(route('register')); ?>" class="flex items-center justify-center w-full h-full p-2">
...
</html><?php /**PATH C:\Users\Youcode\Desktop\playreserve\resources\views/layouts/app.blade.php ENDPATH**/ ?>